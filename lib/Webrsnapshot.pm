...
sub startup ($self) {
    ...
    # Check if used system commands are installed
    ...
    system ("which rsnapshot > /dev/null") == 0 or die "Command: rsnapshot not found on this server!";
    ...
    $config->{rootpass} = $config->{rootpass}?$config->{rootpass}:"{X-PBKDF2}HMACSHA3+256:AAAnEA:rnoT4S2DydQMgg==:ptwlSeP6hmEyWtX54QXCY66zxJ0eFBfNey76LdZ3BwQ=";
